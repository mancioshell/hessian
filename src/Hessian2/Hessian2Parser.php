...
class Hessian2Parser{
	...
	function parse($code = null, $expect = false){
		...
		do {
			...
			$fun = $rule->func;
			$num = ord($code);
			$this->logMsg("Calling $fun with code:$code and num:$num hex:0x".dechex($num). " offset ".$this->stream->pos);
	...
	function string0($code, $num){
		return $this->readHessianString($num);
	}
	
	function string1($code, $num){
		$len = (($num - 0x30) << 8) + ord($this->read());
		return $this->readHessianString($len);
	}
	...
	function readHessianString($len){
        ...
        while($pass < $len) {
            ...
            if($ch < 0x80) {
                ...
                throw new Exception("Hessian string received is not well-formed");
            }
            $pass++;
        }
        $buf = mb_convert_encoding($buf, 'UTF-8', 'UTF-16');
        return $buf;
    }

        
    // Now replaced by readHessianString() (Massimo Squillace 2012-09-07)
	function readUTF8Bytes($len){
        ...
        return $string; // Always return UTF-8 string (Massimo Squillace 2012-08-17)

        /* Commented by Massimo Squillace 2012-08-17
	...
	function untypedMap($code, $num){
		...
		while($code != 'Z'){
			...
			//Risolve 
			if(is_object($key) && $key instanceof QName) $key = implode(",", (array) $key);
